...
def sal(delay_ec):
    """
    EC (Salinity) monitoring function to be run in a separate thread.

    :param delay_ec: Delay between readings in seconds.
    """
    try:
        ec_sensor = Salinity(pin=27)  # Initialize EC sensor with GPIO27
        voltage_1413 = 0.410
        voltage_12880 = 1.990

        ec_sensor.calibrate(voltage_1413, voltage_12880)  # Calibrate with known points
        temperature = 25  # Placeholder temperature; replace with actual reading if available
        ec_sensor.start_continuous_reading(delay=delay_ec, temperature=temperature)
    except Exception as e:
        print(f"Error in EC thread: {e}")
